...
export default function AchievementFormPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'https://www.univpasifik.ac.id'}/api/achievements`, {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        toast({
          title: "Gagal Membuat",
          description: err.error || 'Gagal membuat prestasi',
          variant: "destructive",
        })
      }
    } catch (error) {
      ...
      toast({
        ...
        description: "Gagal membuat prestasi. Silakan coba lagi.",
        variant: "destructive",
      })
    } finally {
      setSubmitting(false)
    ...
    <AdminLayout>
      <div className="min-h-screen bg-unipas-muted py-8">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <h1 className="text-2xl font-bold text-ui-navy mb-4">Tambah Prestasi Baru</h1>
